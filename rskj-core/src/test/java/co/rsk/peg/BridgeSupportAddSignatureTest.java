...
public class BridgeSupportAddSignatureTest extends BridgeSupportTestBase {
    ...
    public void addSignatureMultipleInputsPartiallyValid() throws Exception {
        ...
        for (int i = 0; i < releaseTx.getInputs().size(); i++) {
            ...
            Assertions.assertTrue(Objects.requireNonNull(retrievedScriptSig.getChunks().get(1).data).length > 0);
            Assertions.assertTrue(Objects.requireNonNull(retrievedScriptSig.getChunks().get(2).data).length > 0);
        }
    }
    ...
    private void addSignatureFromValidFederator(List<BtcECKey> privateKeysToSignWith, int numberOfInputsToSign, boolean signatureCanonical, boolean signTwice, String expectedResult) throws Exception {
        ...
        final Keccak256 keccak256 = createHash3();
        ...
        if ("FullySigned".equals(expectedResult)) {
            ...
            Assertions.assertEquals(4, retrievedScriptSig.getChunks().size());
            Assertions.assertTrue(retrievedScriptSig.getChunks().get(1).data.length > 0);
            Assertions.assertTrue(retrievedScriptSig.getChunks().get(2).data.length > 0);
        } else {
            ...
            Assertions.assertFalse(retrievedScriptSig.getChunks().get(2).data.length > 0);
        }
    }
